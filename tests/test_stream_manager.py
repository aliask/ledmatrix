...
class TestReceiveData(unittest.TestCase):
    ...
    @mock.patch("ledmatrix.UDPServer", autospec=True)
    def test_stream_manager_client_sync(self, mock_udpserver, mock_ledmatrix):
        test_image = ImageFrame(pixels=bytearray(b"\xff" * 16 * 32 * 4), height=16, width=32)
        test_image.source =("1.1.1.1", 12345)
        mock_udpserver.get_frame.return_value = test_image
        stream_manger = StreamManager(server=mock_udpserver, leds=mock_ledmatrix)
        stream_manger.run()
        ...
        mock_udpserver.get_frame.side_effect = NoDataException()
        stream_manger.run()
        self.assertEqual(len(stream_manger.streams), 0)

    @mock.patch("ledmatrix.LEDMatrix", autospec=True)
    @mock.patch("ledmatrix.UDPServer", autospec=True)
    def test_stream_manager_switch_to_higher_prio(self, mock_udpserver, mock_ledmatrix):
        test_image = ImageFrame(pixels=bytearray(b"\xff" * 16 * 32 * 4), height=16, width=32)
        test_image.source = ("1.1.1.1", 12345)
        mock_udpserver.get_frame.return_value = test_image
        stream_manger = StreamManager(server=mock_udpserver, leds=mock_ledmatrix)
        ...
        mock_udpserver.get_frame.return_value = test_command
        stream_manger.run()
        active_stream = stream_manger.get_active_stream()
